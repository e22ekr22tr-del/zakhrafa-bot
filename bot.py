...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and context.args[0].startswith("whisper_"):
        ...
        if msg_id in hamsas:
            ...
            hamsas[msg_id]["state"] = "send" # ضفنا state
...
async def handle_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg_id = f"whisper_{chat_id}_{update.message_id}"
    ...
    hamsas[msg_id] = {"sender": sender, "target": target, "chat_id": chat_id, "text": "", "waiting": True, "state": "waiting", "message_id": 0, "reply_id": update.message_id}
    save_json(hamsas, HAMSAS_FILE)
...
async def handle_private_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for msg_id, data in hamsas.items():
        if data.get("sender") == user_id and data.get("state") == "send":
            found_msg_id = msg_id
            break
    ...
    hamsas[found_msg_id]["state"] = "done"
    save_json(hamsas, HAMSAS_FILE)
